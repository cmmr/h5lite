...
try:
    with h5py.File(file_path, "r") as f:
        ...
        # Char (Strings are often bytes in h5py)
        data = f["vec_char"][:]
        ...
        # --- 2. Verify Matrix ---
        # R is column-major, HDF5/Python is row-major.
        # R matrix: 
        # 1 3 5
        # 2 4 6
        # h5lite likely writes dimensions as (2, 3).
        data = f["matrix_int"][:]
        ...
        if "unit" not in dset.attrs or dset.attrs["unit"].decode() != "meters":
            fail(f"Attribute 'unit' mismatch. Got: {dset.attrs.get('unit')}")
        
        if "scale" not in dset.attrs or not np.isclose(dset.attrs["scale"], 1.5):
             fail(f"Attribute 'scale' mismatch. Got: {dset.attrs.get('scale')}")
