...
For an introduction to writing datasets, see `vignette("h5lite")`.
...
See `vignette("matrices")` for more details on this specific case.
...
Unlike matrices, the attributes of a `data.frame` can be safely round-tripped with `h5lite`. This is because its key metadata is stored in a format that `h5lite` can handle. See `vignette("data-frames")` for more on working with data frames.
