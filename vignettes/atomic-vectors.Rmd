...
For details on other data structures, see `vignette("matrices", package = "h5lite")` and `vignette("data-frames", package = "h5lite")`.
...
A key feature of `h5lite` is its automatic and intelligent selection of on-disk data types for numeric data. This is controlled by the `dtype` argument in `h5_write()` and `h5_write_attr()`, which defaults to `"auto"`.
...
R objects can have metadata attached to them as attributes (e.g., the `names` of a named vector). To preserve these during a write/read cycle, you must set `attrs = TRUE`. For a more detailed discussion, see `vignette("attributes-in-depth", package = "h5lite")`.
